...
OUTPUT_DIR = "./drive/MyDrive/gan/" + OUTPUT_NAME
...
def buildDiscriminator(isConv = False):
    if not isConv:
        model = Sequential()
        ...
        # Creating a Keras Model out of the network
        model.add(Conv2D(20, kernel_size=4, strides=1, input_shape=IMAGE_SHAPE, padding="same"))
        model.add(LeakyReLU(alpha = 0.2))
        model.add(Conv2D(10, kernel_size=4, strides=2, padding="same"))
        model.add(LeakyReLU(alpha = 0.2))
        model.add(Flatten())
        model.add(Dense(1, activation="sigmoid"))
        inputTensor = Input(shape = IMAGE_SHAPE)
...
def buildGenerator(isConv = False):
    if not isConv:
        model = Sequential()
        ...
        model = Sequential()
        model.add(Dense(14 * 14 * 64, input_dim = NOISE_SIZE))
...
def buildGAN(images, epochs = 40000, batchSize = 32, loggingInterval = 0, ratio = 1, isConv = False):
    # Setup
    opt = Adam(lr = 0.0002)
    loss = "binary_crossentropy"

    # Setup adversary
    adversary = buildDiscriminator(isConv)
    ...
    generator = buildGenerator(isConv)                    # generator is trained within GAN in relation to adversary performance
    ...
    # Training
    trueCol_g = np.ones((batchSize, 1))
    trueCol_d = np.ones((int(ratio * batchSize), 1))
    falseCol = np.zeros((batchSize, 1))
    for epoch in range(epochs):

        # Train discriminator with a true and false batch
        batch = images[np.random.randint(0, images.shape[0], int(ratio * batchSize))]
        noise = np.random.normal(0, 1, (batchSize, NOISE_SIZE))
        genImages = generator.predict(noise)
        advTrueLoss = adversary.train_on_batch(batch, trueCol_d)
        ...
        noise = np.random.normal(0, 1, (batchSize, NOISE_SIZE))
        genLoss = gan.train_on_batch(noise, trueCol_g)

        save_losses.append((advLoss[0], genLoss))
        # Logging
...
def main():
    ...
    (generator, adv, gan) = buildGAN(data, epochs = 10000, loggingInterval = 1000, ratio = 1.0, isConv = False)
    ...
    xpoints = np.array([i for i in range(len(save_losses))])
    for loss in zip(*save_losses):
      plt.plot(xpoints, loss)
    plt.title("Losses")
